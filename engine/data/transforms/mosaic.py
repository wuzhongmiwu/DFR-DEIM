        ...
        for i, sample in enumerate(mosaic_samples):  
            ...
            if 'masks' in target:
                ...
                if isinstance(target['masks'], torch.Tensor): 
                    ...
                    # 假设masks的形状是 [N, H, W]
                    h, w = target['masks'].shape[1], target['masks'].shape[2]
                    full_masks = torch.zeros((num_instances, max_height * 2, max_width * 2),    
                                            dtype=target['masks'].dtype)
                    # 将mask放置到对应位置    
                    y_offset, x_offset = placement_offsets[i][1], placement_offsets[i][0]
                    full_masks[:, y_offset:y_offset+h, x_offset:x_offset+w] = target['masks']     
                    target['masks'] = full_masks     
                else:
                    # 如果是PIL Image或numpy array格式     
            ...
            if key == 'boxes':    
                ...
                for i, target in enumerate(targets):
                    if isinstance(target[key], torch.Tensor):    
                        num_instances = target[key].shape[0]
                        h, w = target[key].shape[1], target[key].shape[2]    
    
                        # 创建完整大小的mask画布 
                        full_masks = torch.zeros((num_instances, max_height * 2, max_width * 2),   
                                                dtype=target[key].dtype, device=target[key].device)  
   
                        # 将mask放置到对应位置    
                        y_offset, x_offset = placement_offsets[i][1], placement_offsets[i][0]
                        full_masks[:, y_offset:y_offset+h, x_offset:x_offset+w] = target[key] 
                        
                        values.append(full_masks)   
                    else:
                        raise NotImplementedError("Please convert masks to torch.Tensor format")
            else:
                values = [target[key] for target in targets]
