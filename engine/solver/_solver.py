...
class BaseSolver(object):
    ...
    def _setup(self, eval=False): 
        ...
        logger.info(RED  + f"output_dir:{str(self.output_dir)}" + RESET)  
        if dist_utils.is_main_process():     
    ...
    def state_dict(self):  
        ...
        if self.train_dataloader.dataset.remap_mscoco_category: 
            state['name'] = {mscoco_category2label[i]:k for i, k in self.train_dataloader.dataset.category2name.items()}
    ...
    def load_resume_state(self, path: str):
        ...
        # 过滤掉统计相关的键
        state['model'] = {k: v for k, v in state['model'].items() if not any(x in k for x in ['total_ops', 'total_params'])}
        ...
        for param_name in head_param_names:
            if param_name in cur_state_dict and param_name in pretrain_state_dict: 
                ...
                if adjusted_tensor is not None:    
                    ...
                    logger.warning(f"Cannot adjust parameter '{param_name}' due to size mismatch.")
...
class ModelSaverFunc:     
    def __init__(self, trainer):
        self.trainer = trainer 
    
    def __enter__(self):   
        self.trainer.criterion.reset_model()
   
    def __exit__(self, *args):
        self.trainer.criterion.set_model(self.trainer.model)  
        return False     
