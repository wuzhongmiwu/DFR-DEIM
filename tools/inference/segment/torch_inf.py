...
def process_video(model, device, file_path, output_path, thrh, track):
    ...
    if cap.isOpened():
        for _ in tqdm.tqdm(range(total_frames), desc='Processing video frames...'):
            ...
            if track:
                track_res = track_instance(np.array(frame_pil), (labels, boxes, scores)).to(labels.device)
                # Draw detections on the frame
                frame_pil = draw([frame_pil], labels, boxes, scores, [masks[0, track_res[:, 7].int()]], tracks=[track_res], thrh=thrh, class_name=CLASS_NAME)
            else:
                frame_pil = draw([frame_pil], labels, boxes, scores, masks, thrh=thrh, class_name=CLASS_NAME)
