...
#include "cuda/dscn_im2col_cuda.cuh"
#include <vector>
...
at::Tensor dscn_cuda_forward(const at::Tensor &input, const at::Tensor &offset,
    ...
    auto per_offset_size =
        height_out * width_out * group * (kernel_h * kernel_w - remove_center);
    for (int n = 0; n < batch / im2col_step_; ++n) {
        auto columns = output_n.select(0, n);
        // AT_DISPATCH_FLOATING_TYPES(
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(
            input.scalar_type(), "deform_strip_conv_forward_cuda", ([&] {
                dscn_im2col_cuda(
                    ...
                    width_out, offset_scale, remove_center, on_x);
            }));
    }
...
dscn_cuda_backward(const at::Tensor &input, const at::Tensor &offset,
    ...
    for (int n = 0; n < batch / im2col_step_; ++n) {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(
            input.scalar_type(), "deform_strip_conv_backward_cuda", ([&] {
                dscn_col2im_cuda(
                    at::cuda::getCurrentCUDAStream(),
                    ...
                    grad_offset.data<opmath_t>() +
                        n * im2col_step_ * per_offset_size, on_x);
            }));
    }
